...
-   [Landre et al (2024)](https://doi.org/10.1016/j.esmoop.2023.102220) conducted a meta-analysis to answer this question more definitively than any given single study
...
-   You can fit a linear model with an interaction term
-   Create a dataframe consisting of the effect estimates and standard errors using `emmeans::emtrends()`
...
#Load libraries with the pacman package
library(tidyverse)
library(emmeans)

#Read in the stroke dat
stroke <- read.csv("stroke.csv")

#Make single age group variable
stroke <- stroke %>%
  mutate(age_group = case_when(
    Age_less_eq54 == 1 ~ "Age <= 54",
    Age_55to64 == 1 ~ "Age 55-64",
    Age_65to74 == 1 ~ "Age 65-74",
    Age_ge75 == 1 ~ "Age >= 75"
  ))

stroke$age_group <- factor(
  stroke$age_group,
  levels = c("Age <= 54", "Age 55-64", "Age 65-74", "Age >= 75")
)

#Fit linear regression model with interaction term
mod <- lm(Depression ~ Fatalism * age_group, data = stroke)

#Use emmeans package to extract subgroup effects
emm <- emtrends(mod, ~ age_group, var = "Fatalism")

#Make a nice dataframe for plotting
results <- as.data.frame(emm)

results <- results %>%
  rename(
    estimate = Fatalism.trend,
    se = SE,
    lwr = lower.CL,
    upr = upper.CL
  )
...
ggplot(results, aes(x = estimate, y = age_group)) +
  geom_point(size = 2) +
  geom_segment(aes(x = lwr, xend = upr)) +
  labs(
    title = "Effect of Fatalism on Depression Score by Age Group",
    x = "Estimate",
    y = "Age Group"
  ) +
  xlim(-0.8, 0.8) +
